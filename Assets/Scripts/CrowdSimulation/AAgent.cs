...
namespace CrowdSimulation
{
    public class AAgent : Agent
    {
        ...
        [SerializeField] bool navmeshDebug = false;          // Use navigation mesh to get intermediate waypoints
        ...
        void Start()
        {
            ...
            if (navmesh)
            {
                var tmp = GetWaypoints(transform.GetChild(0), GetComponent<NavMeshAgent>());
                goals = new Queue(tmp);
                if (goals.Count < 1) return;
                goals.Dequeue();
                goal3d = (Vector3) goals.Dequeue();
                goal = ExtensionMethods.ToXZ(goal3d);

                if (navmeshDebug) {
                    for (int i = 1; i < tmp.Length; i++)
                    {
                        Vector3 p = new Vector3(tmp[i].x, 0, tmp[i].z);
                        Instantiate(prefab, p, Quaternion.identity, transform.parent);
                    }
                }
            }
    ...
    void Evaluate()
        ...
        public void MoveInRealWorld()
        {
            ...
            if (distanceToTarget < goalRadius * 1.2f)
            {
                if (goals.Count < 1) return;
                goal3d = (Vector3)goals.Dequeue();
