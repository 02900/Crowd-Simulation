    ...
    public static class Groups
    {
            ...
            out Point fl1, out Point fl2, out Point fr1, out Point fr2)
        {
            ...
            for (int i = 0; i < tangentsPoints.Count; i++)
            {
                ...
                if (debug)
                {
                    ...
                    ExtensionMethods.DrawLine(v3_t1, v3_t2, 0.75f, new Color(1, 1, 1, 0.4f));
            ...
            out float gRad, out int turnTo, out List<int> members, bool debug)
        {
            ...
            if (debug)
            {
                ...
                ExtensionMethods.DrawLine(new Vector3(I.x, 1.5f, I.y), new Vector3(fl2.x, 1.5f, fl2.y), 0.75f, Color.white);
                ExtensionMethods.DrawLine(new Vector3(I.x, 1.5f, I.y), new Vector3(fr2.x, 1.5f, fr2.y), 0.75f, Color.white);
        ...
        static void DrawCircle(float radius, Vector2 position) {
            ...
            for (f_t4 = 0.1f; f_t4 < Mathf.PI * 2; f_t4 += 0.1f)
            {
                ...
                ExtensionMethods.DrawLine(v3_t1, v3_t2, 0.75f, Color.white);
                v3_t1 = v3_t2;
            }
            //Debug.DrawLine(v3_t1, v3_t3);
            ExtensionMethods.DrawLine(v3_t1, v3_t3, 0.75f, Color.white);
        }
    }
}
